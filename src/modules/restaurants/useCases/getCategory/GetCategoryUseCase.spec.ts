...
describe("Get Category", () => {
	...
	it("should be able to get a category of a inexistent restaurant", async () => {
		const user = await createUserUseCase.execute({
			name: "User Name",
			email: "user@email.com",
			password: "password",
		...
		await expect(
			getCategoryUseCase.execute(user.id, "invalid id", category.id)
		).rejects.toEqual(new AppError("Invalid restaurant ID."));
	});

	it("should be able to get a category of a restaurant that does not belong to the user", async () => {
		...
		await expect(
			getCategoryUseCase.execute(user2.id, restaurant.id, category.id)
		).rejects.toEqual(
			new AppError("Restaurant does not belong to this user.")
		);
	});

	it("should be able to get a inexistent category", async () => {
		const user = await createUserUseCase.execute({
			name: "User Name",
			email: "user@email.com",
			password: "password",
		...
		await expect(
			getCategoryUseCase.execute(user.id, restaurant.id, "invalid id")
		).rejects.toEqual(new AppError("Invalid category ID."));
	});

	it("should be able to get a category that does not belong to the restaurant", async () => {
		const user = await createUserUseCase.execute({
			name: "User Name",
			email: "user@email.com",
			password: "password",
		...
		expect(
			getCategoryUseCase.execute(user.id, restaurant2.id, category.id)
		).rejects.toEqual(
			new AppError("Category does not belong to the restaurant.")
		);
	});
});
