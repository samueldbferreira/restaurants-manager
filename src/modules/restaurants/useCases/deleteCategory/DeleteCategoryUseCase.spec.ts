...
describe("Delete Category", () => {
	...
	it("should not be able to delete a category of a inexistent restaurant", () => {
		expect(async () => {
			const user = await createUserUseCase.execute({
				name: "User Name",
				email: "user@email.com",
			...
			await deleteCategoryUseCase.execute(user.id, "invalid id", category.id);
		}).rejects.toBeInstanceOf(AppError);
	});

	it("should not be able to delete a category of a restaurant that does not belong to the user", () => {
		expect(async () => {
			const user = await createUserUseCase.execute({
				name: "User Name",
				email: "user@email.com",
			...
			await deleteCategoryUseCase.execute(user2.id, restaurant.id, category.id);
		}).rejects.toBeInstanceOf(AppError);
	});
	...
	it("should not be able to delete a category that does not belong to the restaurant", () => {
		expect(async () => {
			const user = await createUserUseCase.execute({
				name: "User Name",
				email: "user@email.com",
			...
			await deleteCategoryUseCase.execute(user.id, restaurant2.id, category.id);
		}).rejects.toBeInstanceOf(AppError);
	});
});
