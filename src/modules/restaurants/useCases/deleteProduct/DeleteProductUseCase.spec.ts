...
describe("Delete Product", () => {
	...
	it("should not be able to delete a product of a inexistent restaurant", async () => {
		const user = await createUserUseCase.execute({
			name: "User Name",
			email: "user@email.com",
			password: "password",
		...
		await expect(
			deleteProductUseCase.execute(user.id, "invalid id", product.id)
		).rejects.toEqual(new AppError("Invalid restaurant ID."));
	});

	it("should not be able to delete a product of a restaurant that does not belong to the user", async () => {
		const user = await createUserUseCase.execute({
			name: "User Name",
			email: "user@email.com",
			password: "password",
		...
		await expect(
			deleteProductUseCase.execute(user2.id, restaurant.id, product.id)
		).rejects.toEqual(new AppError("Restaurant does not belong to this user."));
	});

	it("should not be able to delete a product that does not belong to the restaurant", async () => {
		const user = await createUserUseCase.execute({
			name: "User Name",
			email: "user@email.com",
			password: "password",
		...
		await expect(
			deleteProductUseCase.execute(user.id, restaurant2.id, product.id)
		).rejects.toEqual(
	...
	it("should not be able to delete a non-existing product", async () => {
		...
		await expect(
			deleteProductUseCase.execute(user.id, restaurant.id, "invalid product id")
		).rejects.toEqual(new AppError("Product does not exist.", 404));
	});
});
