...
describe("Get Sale", () => {
	...
	it("should not be able to get a sale from a inexistent restaurant", async () => {
		const user = await createUserUseCase.execute({
			name: "User Name",
			email: "user@email.com",
			password: "password",
		...
		await expect(
			getSaleUseCase.execute(user.id, "invalid id", sale.id)
		).rejects.toEqual(new AppError("Invalid restaurant ID."));
	});

	it("should not be able to get a sale from a restaurant that does not belong to this user.", async () => {
		const user = await createUserUseCase.execute({
			name: "User Name",
			email: "user@email.com",
			password: "password",
		...
		await expect(
			getSaleUseCase.execute(user2.id, restaurant.id, sale.id)
		).rejects.toEqual(new AppError("Restaurant does not belong to this user."));
	});

	it("should not be able to get a inexistent sale", async () => {
		const user = await createUserUseCase.execute({
			name: "User Name",
			email: "user@email.com",
			password: "password",
		...
		await expect(
			getSaleUseCase.execute(user.id, restaurant.id, "invalid id")
		).rejects.toEqual(new AppError("Invalid sale ID."));
	});

	it("should not be able to get a sale that does not belong to the restaurant", async () => {
		const user = await createUserUseCase.execute({
			name: "User Name",
			email: "user@email.com",
			password: "password",
		...
		await expect(
			getSaleUseCase.execute(user.id, restaurant2.id, sale.id)
